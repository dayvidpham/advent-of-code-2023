...
lineToParts
  ...
  dict = do
    ...
    --print btmss
    --putStrLn ""
    --putStrLn $ concat [show (row, col), "\t", show nextRowCol]
    --putStrLn $ concat [show $ take (endCheckCol-startCheckCol) $ drop dropN tops, "\t", show $ indexOf '*' checkTops 1]
    --putStrLn $ concat [show $ take (endCheckCol-startCheckCol) $ drop dropN curs, "\t", show $ indexOf '*' (concat [preChar, numStr, postNum]) 1]
    --putStrLn $ concat [show $ take (endCheckCol-startCheckCol) $ drop dropN btms, "\t", show $ indexOf '*' checkBtms 1]
    --print checkIdx
